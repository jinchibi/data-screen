...
export default function () {
  ...
  onMounted(() => {
    task = setInterval(() => {
      ...
      _headerData.headerData.value.forEach(item => {
        item.endVal += random(100)
      })
      _headerData.project.value.forEach(item => {
        item.value = `${(+item.value.slice(0, item.value.length - 1) + random(5)).toFixed(2)}%`
